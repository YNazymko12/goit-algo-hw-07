...
def get_min_value_node(node):
    ...
    while current.left is not None:
        # Продовжуємо рухатися до лівого піддерева, поки не знайдемо найменше значення
...
def get_max_value_node(node):
    ...
    while current.right is not None:
        # Продовжуємо рухатися до правого піддерева, поки не знайдемо найбільше значення
        current = current.right
    return current  # Повертаємо вузол з максимальним значенням
...
def delete_node(root, key):
    ...
    if key < root.key:
        ...
        if root.left is None:
            # Вузол має лише праве піддерево або немає дітей
            temp = root.right
            root = None
            return temp
        elif root.right is None:
            # Вузол має лише ліве піддерево або немає дітей
            temp = root.left
            root = None
            return temp
